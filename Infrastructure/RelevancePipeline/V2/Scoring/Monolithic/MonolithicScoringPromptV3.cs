...
public static class MonolithicScoringPromptV3
{
    public const string Version = "scoring_monolithic_v3_11_source_weighting";

    public static string Build(string cvSummaryJson, string vacancyRawText)
    {
        var sb = new StringBuilder(4000);
        sb.Append("Score how well the CV matches the job. Return 7 sub-scores and an anti-flag penalty.\n");
        sb.Append("\n");
        ...
        sb.Append("- You will be invoked on ~30-90 similar vacancies in one batch. Two vacancies that differ in\n");
        sb.Append("  ANY observable way (one extra must-have, slightly different seniority signal, different\n");
        sb.Append("  domain, different team-size signal) MUST receive sub-scores that differ by at least 0.02 on\n");
        sb.Append("  the axes where the difference lives. Identical sub-score vectors across distinct vacancies\n");
        sb.Append("  are a SCORING BUG — vary by the smallest honest delta you can justify.\n");
        ...
        sb.Append("                       This prevents a CV that lists methodologies in a course bullet\n");
        sb.Append("                       from scoring full-credit against a Senior vacancy.\n");
        sb.Append("\n");
        ...
        sb.Append("                       Treat these adjacent sub-domains as a ramp-up risk (~6 months\n");
        sb.Append("                       to learn the new sub-stack). Do not round adjacent sub-domains\n");
        sb.Append("                       to 1.0 just because the role title matches.\n");
        sb.Append("\n");
        ...
        sb.Append("\n");
        sb.Append("TRIGGER 0.2 (HARD blocker — these are the only narrow categories where you go hard):\n");
        sb.Append("  - Military service / mobilization: ONLY explicit phrases such as \"мобілізація\",\n");
        sb.Append("    \"призов\", \"служба за контрактом ЗСУ\", \"військова частина\", \"бригада ЗСУ\",\n");
        sb.Append("    \"бойові бригади\", named combat units, or English equivalents (\"mandatory military\n");
        sb.Append("    enlistment\", \"combat service contract\"). General \"defence-industry product\" or\n");
        sb.Append("    \"DefTech startup building drones for army\" is NOT enough — must be enlistment.\n");
        sb.Append("  - Unpaid / volunteer / pro-bono positions.\n");
        sb.Append("  - Citizenship restriction the CV literally cannot satisfy (\"US citizens only\" + CV\n");
        sb.Append("    nationality clearly different).\n");
        sb.Append("\n");
        ...
        sb.Append("  - Specific timezone overlap stated in the vacancy (e.g. \"must overlap with US PST 9-5\",\n");
        sb.Append("    \"EST business hours\") — only trigger when the CV explicitly indicates a different\n");
        sb.Append("    timezone OR when the time delta with Kyiv is severe (>=6 hours).\n");
        sb.Append("  - \"contract-only\" / \"freelance-only\" / \"part-time only\" — only when the vacancy is\n");
        sb.Append("    unambiguous about this being the ONLY mode (e.g. \"this is a 6-month contract, no\n");
        sb.Append("    full-time conversion\").\n");
        sb.Append("\n");
        ...
        sb.Append("\n");
        sb.Append("When you trigger, list the literal phrase from the vacancy text in `triggered_anti_flags`.\n");
        ...
        sb.Append("\n");
        sb.Append("  1.0  — both CV and vacancy are detailed, requirements are explicit, skill overlap is\n");
        sb.Append("         unambiguous either way. You would defend the score under scrutiny.\n");
        sb.Append("  0.8  — minor ambiguity (one or two skills you had to canonicalise; mild seniority gap),\n");
        sb.Append("         but the overall picture is clear.\n");
        sb.Append("  0.6  — vacancy or CV is partly vague (short description, generic role title,\n");
        sb.Append("         no years specified). The score is your best guess, not a certainty.\n");
        sb.Append("  0.4  — substantial missing information: very short vacancy, no must-have list,\n");
        sb.Append("         CV missing key sections. You would flag this pair for human review.\n");
        sb.Append("  0.2  — almost no information to work with (1-2 sentences in vacancy, near-empty CV).\n");
        sb.Append("\n");
        sb.Append("Lowering confidence does NOT change the sub_scores. It only tells the downstream system\n");
        sb.Append("that this result should be treated as uncertain.\n");
        sb.Append("\n");
        sb.Append("# REASON STYLE — write like a human, not like a system log\n");
        sb.Append("Audience: the job-seeker is reading this on their phone deciding whether to\n");
        sb.Append("apply. Talk TO them, not ABOUT them.\n");
        sb.Append("\n");
        sb.Append("Voice rules:\n");
        sb.Append("- Use second person. English: \"you bring\", \"your background\". Ukrainian: \"ваш\n");
        sb.Append("  досвід\", \"ви маєте\". Address the candidate directly.\n");
        sb.Append("- Conversational tone, not corporate jargon. Vary the opening across pairs —\n");
        sb.Append("  do NOT start every reason with \"Strong skills in X\" / \"Сильні навички X\".\n");
        sb.Append("  Some good openings: \"Your X experience fits this stack well — but…\",\n");
        sb.Append("  \"Гарне попадання по X, проте…\", \"This role lines up with…\",\n");
        sb.Append("  \"Тут все непогано з X, але є нюанс — …\".\n");
        sb.Append("- Pick the SINGLE most-relevant strength and the SINGLE most-important gap (or\n");
        ...
        sb.Append("- Mention concrete things from the CV/vacancy by name (e.g. \"ASP.NET Core 8 +\n");
        sb.Append("  microservices\", \"WinForms\", \"US Citizenship\"). Avoid generic phrases like\n");
        sb.Append("  \"good fit\", \"strong match\", \"відмінний збіг\", \"хороші навички\".\n");
        sb.Append("- Length: 25-45 words per language. Two sentences usually — one for the fit,\n");
        sb.Append("  one for the gap or blocker. If the pair is a clean No, one sentence is fine.\n");
        sb.Append("\n");
        ...
        sb.Append("  RIGHT: \"The catch — this is a uniformed military role, not a civilian contract,\n");
        sb.Append("          which is a hard left turn from your current career.\"\n");
        sb.Append("  WRONG: \"Спрацював анти-флаг громадянства США.\"\n");
        sb.Append("  RIGHT: \"Мінус — позиція тільки для громадян США / резидентів, тож формально\n");
        sb.Append("          ви не пройдете легальну перевірку, навіть з відповідним стеком.\"\n");
        sb.Append("\n");
        sb.Append("Worked examples (for style reference, NOT to copy verbatim):\n");
        sb.Append("\n");
        sb.Append("Strong fit example —\n");
        sb.Append("  reason_en: \"Your 6 years on .NET microservices + Postgres lines up cleanly with\n");
        sb.Append("              the team's stack, and the seniority bracket matches. The only mild\n");
        sb.Append("              gap is contract testing — they use Pact, you've mentioned k6 only.\"\n");
        sb.Append("  reason_uk: \"Шість років з .NET-мікросервісами та Postgres лягають у їх стек\n");
        sb.Append("              майже без зазорів, рівень теж збігається. Невеликий нюанс — у них\n");
        sb.Append("              контракт-тести на Pact, у вас згадано тільки k6.\"\n");
        sb.Append("\n");
        sb.Append("Overqualified example —\n");
        sb.Append("  reason_en: \"Technically you can do this with one hand — but it's a Junior\n");
        sb.Append("              under-direct-supervision role, and you're already mentoring middles.\n");
        sb.Append("              You'd outgrow it in three months.\"\n");
        sb.Append("  reason_uk: \"Технічно зробите з закритими очима — проте це Junior під прямим\n");
        sb.Append("              супервайзингом, а ви вже менторите мідлів. Переростете за три місяці.\"\n");
        sb.Append("\n");
        sb.Append("Hard blocker example —\n");
        sb.Append("  reason_en: \"Stack-wise you're well above the bar. The deal-breaker is the\n");
        sb.Append("              mandatory relocation to Georgia plus US-citizenship gate — no path\n");
        sb.Append("              forward from Kyiv.\"\n");
        sb.Append("  reason_uk: \"По стеку ви явно вище планки. Зупиняє інше — обов'язковий переїзд\n");
        sb.Append("              у Джорджію плюс вимога громадянства США, що з Києва не обійти.\"\n");
        sb.Append("\n");
        ...
        sb.Append("  \"reason_en\": string (see REASON STYLE above — 25-45 words, second person),\n");
        sb.Append("  \"reason_uk\": string (see REASON STYLE above — 25-45 words, second person)\n");
        sb.Append("}\n");
